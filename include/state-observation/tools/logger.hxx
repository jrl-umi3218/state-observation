...
void Logger::record(const T * address, const std::string & filename)
{
  log_s log(filename);
  log.setType<T>();
  if(*log.type != typeid(Matrix) && *log.type != typeid(Matrix3) && *log.type != typeid(Vector)
     && *log.type != typeid(Vector3) && *log.type != typeid(int) && *log.type != typeid(long)
     && *log.type != typeid(double) && *log.type != typeid(float))
  {
    BOOST_ASSERT(false && "Tentative to record an unsupported type");
    throw std::invalid_argument(
        (std::string("The type ") + std::string("log.type->name") + std::string(" is not supported")).c_str());
  }
